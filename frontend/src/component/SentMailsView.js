const SentMailView = (props) => {
  const title = <div class="section_title">Sent Mails</div>;
  const Mails = props.sentMails.sort((x, y) => {return  y.created_at - x.created_at})
  Mails.map(mail => mail.created_at =new Date(mail.created_at))
  const view = Mails.map(({  id,  reciverId, text , created_at }) => (
    <div class="mail" key={id}>
      <div class="mail_dis">To:{reciverId}</div>
      <div class="mail_text">
